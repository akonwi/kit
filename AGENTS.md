...
5. For changes to one-shot/headless execution, run **`bun run smoke:one-shot`** from the repository root. This is an authenticated manual smoke test that makes model calls and exercises prompts, tools, external-plugin exclusion, signals, subagents, and ephemeral storage.
