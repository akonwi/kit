...
export type McpStatusContentProps = {
...
export function McpStatusContent(props: McpStatusContentProps) {
	return (
		<Show
						...
						{(state) => {
							...
							return (
								<box flexDirection="column" gap={0}>
									<box flexDirection="row" justifyContent="space-between">
										<box flexDirection="row" gap={1}>
											<Show
												...
												fallback={<Spinner fg={statusIndicator(state).color} />}
											>
									...
									<Show when={state.toolCount > 0 || state.cached || oauth()}>
									...
									<Show when={state.description}>
										<text fg={theme.textSecondary}>{state.description}</text>
									</Show>
									<text fg={theme.textMuted}>
										{state.source} · {state.filePath}
									</text>
									<Show when={state.lastError}>
										<text fg={theme.errorText}>error: {state.lastError}</text>
									</Show>
								</box>
							);
						}}
