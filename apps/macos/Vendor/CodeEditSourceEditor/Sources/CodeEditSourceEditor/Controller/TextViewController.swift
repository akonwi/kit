...
public class TextViewController: NSViewController {
    ...
    /// Forward vertical wheel events when this editor is embedded in a continuous document.
    public var forwardsVerticalScrollToParent = false {
        didSet { (scrollView as? EmbeddedSourceScrollView)?.forwardsVerticalScrollToParent = forwardsVerticalScrollToParent }
    }

    /// Optional rail width for clients displaying semantic old/new line numbers.
    public var gutterWidthOverride: CGFloat? { didSet { updateTextInsets() } }

    var textViewInsets: HorizontalEdgeInsets {
        HorizontalEdgeInsets(
            left: gutterWidthOverride ?? (showGutter ? gutterView.frame.width : 0.0),
