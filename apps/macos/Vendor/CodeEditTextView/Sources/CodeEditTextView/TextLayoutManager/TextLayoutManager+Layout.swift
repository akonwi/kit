...
extension TextLayoutManager {
    ...
    public func layoutLines(in rect: NSRect? = nil) -> Set<TextLine.ID> { // swiftlint:disable:this function_body_length
        ...
        for linePosition in linesStartingAt(minY, until: maxY).lazy {
            ...
            let lineNotEntirelyLaidOut = linePosition.height != linePosition.data.lineFragments.height + (blockHeights[linePosition.index] ?? 0)
    ...
    private func layoutLine(
        ...
        let totalHeight = lineSize.height + (blockHeights[linePosition.index] ?? 0)
        let wasLineHeightChanged = totalHeight != linePosition.height
