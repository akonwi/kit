...
public class TextLayoutManager: NSObject {
    ...
    /// Extra vertical space after logical lines, without modifying source text.
    public var blockHeights: [Int: CGFloat] = [:] {
        didSet {
            guard blockHeights != oldValue else { return }
            for index in Set(blockHeights.keys).union(oldValue.keys) {
                guard let line = lineStorage.getLine(atIndex: index) else { continue }
                let delta = (blockHeights[index] ?? 0) - (oldValue[index] ?? 0)
                lineStorage.update(atOffset: line.range.location, delta: 0, deltaHeight: delta)
            }
            setNeedsLayout()
        }
    }
    /// Called after source layout and document-height updates have completed.
    public var didLayoutBlocks: (() -> Void)?
