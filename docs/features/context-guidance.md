...
Unreadable, non-regular, invalid UTF-8, oversized, and over-budget candidates produce structured reload diagnostics without preventing an otherwise valid session from opening. The native TUI reports reload completion and diagnostics through the reload result toast.
