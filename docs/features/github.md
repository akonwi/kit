...
Kit has lightweight built-in GitHub awareness through an internal plugin when the GitHub CLI (`gh`) is available and authenticated.
...
- The bottom status footer displays the pull request number alongside VCS information, for example:
...
- PR metadata is resolved asynchronously by the GitHub plugin and cached briefly per branch to avoid calling `gh` on every local VCS update.
