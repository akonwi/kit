...
session discovery and immutable `/api/sessions/<id>/rpc` WebSocket bindings.
`SessionClient` consumes the same sequenced snapshot/replay protocol, negotiates
limits, and reconnects from its last reduced cursor. Normal interactive and
print modes have not migrated to this client path yet.
