...
function decodeBase64(value: string): Uint8Array {
	...
	if (remainder === 1 || (compact.includes("=") && remainder !== 0)) {
		throw new Error("Invalid base64 payload");
	}
	const normalized = compact.padEnd(
		compact.length + ((4 - remainder) % 4),
		"=",
	);
	if (
	...
	for (let index = 0; index < normalized.length; index += 4) {
		...
		const c =
			normalized[index + 2] === "="
				? 0
				: BASE64_ALPHABET.indexOf(normalized[index + 2] ?? "");
		const d =
			normalized[index + 3] === "="
				? 0
				: BASE64_ALPHABET.indexOf(normalized[index + 3] ?? "");
		const group = (a << 18) | (b << 12) | (c << 6) | d;
		if (offset < bytes.length) bytes[offset++] = group >> 16;
