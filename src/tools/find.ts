      ...
      return new Promise((resolve_, reject) => {
        ...
        try {
          ...
          if (relativized.length === 0) {
            resolve_({ content: [{ type: "text", text: "No files found matching pattern" }], details: {} });
            return;
          }
          ...
          if (resultLimitReached) notices.push(`${effectiveLimit} results limit reached — use limit=${effectiveLimit * 2} or refine pattern`);
          if (bytesTruncated) notices.push(`${MAX_BYTES / 1024}KB output limit reached`);
          if (notices.length > 0) output += `\n\n[${notices.join(". ")}]`;

          resolve_({
            content: [{ type: "text", text: output }],
            details: { count: relativized.length, resultLimitReached, bytesTruncated },
          });
        } catch (err) {
          cleanup();
          reject(err);
...
function tryFd(pattern: string, searchPath: string, limit: number): string[] | null {
  ...
  const result = spawnSync("fd", args, { encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 });
