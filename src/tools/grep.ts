			...
			return new Promise((resolve_, reject) => {
				...
				(async () => {
					...
					child.on("error", (err) => {
						...
						rl.close();
						reject(
							new Error(`rg not available: ${err.message}. Install ripgrep.`),
						);
					});
					...
					child.on("close", async (code) => {
						...
						for (const { filePath, lineNumber } of matches) {
							...
							const start =
								contextLines > 0
									? Math.max(1, lineNumber - contextLines)
									: lineNumber;
							const end =
								contextLines > 0
						...
						if (matchLimitReached)
							notices.push(
								`${effectiveLimit} match limit reached — use limit=${effectiveLimit * 2} for more or refine pattern`,
							);
						if (bytesTruncated)
							notices.push(`${MAX_BYTES / 1024}KB output limit reached`);
						if (linesTruncated)
							notices.push(
								`some lines truncated to ${MAX_LINE_LENGTH} chars — use read tool for full lines`,
							);
